...
def log_validation(
    ...
    # Handle FaceID embeddings for IP-Adapter
    validation_faceid_embeddings = None
    if use_ip_adapter_pipeline:
        if args.validation_faceid_embedding is None:
            raise ValueError("`validation_faceid_embedding` must be provided when `enable_ip_adapter` is True and using IP-Adapter pipeline.")
    ...
    for idx, (validation_prompt, validation_image) in enumerate(zip(validation_prompts, validation_images)):
        ...
        # Load FaceID embedding if IP-Adapter pipeline is being used
...
def parse_args(input_args=None):
    ...
    if args.enable_ip_adapter and args.validation_faceid_embedding is not None:
